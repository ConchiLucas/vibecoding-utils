...
export default function DeployLogPanel({ projectId, projectName, envKey, routeName, mode = 'deploy', onClose }: DeployLogPanelProps) {
  ...
  useEffect(() => {
    ...
    const streamEndpoint = mode === 'logs' ? 'dockerLogStream' : mode === 'stop' ? 'stopStream' : 'deployStream';
  ...
  const statusConfig = {
    ...
    running:    { icon: <Loader2 size={14} className="animate-spin" />, text: isDockerLogs ? '实时跟踪中...' : isStop ? '关闭中...' : '部署中...', color: isDockerLogs ? 'text-emerald-400' : isStop ? 'text-orange-500' : 'text-blue-600' },
    success:    { icon: <CheckCircle2 size={14} />, text: isDockerLogs ? '日志已断开' : isStop ? '关闭成功' : '部署成功', color: 'text-green-600' },
    error:      { icon: <XCircle size={14} />, text: isDockerLogs ? '日志读取失败' : isStop ? '关闭失败' : '部署失败', color: 'text-red-600' },
  };
      ...
      <div className="bg-gray-900 rounded-2xl shadow-2xl w-[800px] max-w-[90vw] max-h-[80vh] flex flex-col border border-gray-700 animate-in zoom-in-95 duration-300">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-700/50">
          <div className="flex items-center gap-3">
            <div className={`p-1.5 ${isDockerLogs ? 'bg-emerald-500/20 text-emerald-300' : isStop ? 'bg-orange-500/20 text-orange-400' : 'bg-green-500/20 text-green-400'} rounded-lg`}>
              <Terminal size={16} />
            </div>
            <div>
